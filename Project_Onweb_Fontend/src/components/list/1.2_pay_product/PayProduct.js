...
function ReceiveProduct() {
  ...
  return (
    <div className="container my-5 content bgcolor">
      ...
      {/* แสดงการยืนยันลบ */}
      {showDeleteConfirm && (
        <div className="alert alert-warning alert-dismissible fade show alert-center" role="alert">
          <strong>คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลนี้?</strong>
          <div>
            <button className="btn btn-danger btn-sm me-2" onClick={confirmDelete}>ยืนยัน</button>
            <button className="btn btn-secondary btn-sm" onClick={cancelDelete}>ยกเลิก</button>
          </div>
        </div>
      )}
